...
import { View, Text, StyleSheet, ActivityIndicator, TextInput, TouchableOpacity, Pressable, Alert, RefreshControl, } from 'react-native';
...
const ListsScreen = () => {
    ...
    },[shoppingListsFetched, refreshing]);
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
            <View style={styles.listContainer}>
                ...
                {(!listsDataLoading && listsData?.length > 0) ?
                    (listsData?.filter((list)=> list?.name?.toLowerCase().includes(searchQuery?.toLowerCase()))?.map((list) => {
                        return (
                            <Link href={`/(modals)/list?listId=${list?._id}`} asChild key={list?._id}>
                                <Pressable>
                                    <View style={[styles.listCard,list.isDeleted ? {backgroundColor:'#f8bbbbff'}:{backgroundColor:Colors.light.white}]}>
                                        <View style={[styles.row,list.isDeleted ? {backgroundColor:'#f8bbbbff'}:{backgroundColor:Colors.light.white}]}>
                                        ...
                                        {list.isDeleted && (<Text>This list has been Deleted</Text>)}
                                    </View>
                                </Pressable>
                            </Link>
                        );
