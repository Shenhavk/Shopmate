...
router.patch('/list/:id', async (req, res) => {
  try {
    ...
    const { storeName, branch, total = 0, items = [] } = req.body;
    const list = await ShoppingList.findById(id).populate('items.lastModifiedBy','name').populate('items.history.userId','name add');
...
router.patch('/list/soft_delete/:id', async (req, res) => {
  try {
    const { id } = req.params;
...
router.patch('/list/restore/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const list = await ShoppingList.findById(id);
...
router.post('/list/:id/item/purchase/all', async (req, res) => {
  try {
    const { id } = req.params;
...
router.patch('/list/:listId/item/:itemId/price', async (req, res) => {
  ...
  const { storeName, price, currency = 'ILS' } = req.body;
    ...
    const list = await ShoppingList.findOneAndUpdate(
      ...
      },
      {
        $set: {
          ...
          'items.$.lastModifiedBy': req.user._id,
          $push: {
            'items.$.history': {
              action: 'edit',
              userId: req.user._id,
